...
SMILES_file = 'data_WS/drug_SMILES_750.csv'
mask_mat_file = './data_WS/mask_mat_750.mat'
side_effect_label = 'data_WS/side_effect_label_750.mat'
input_dim = 109
...
def loss_fun(output, label, lam, eps):
    ...
    return loss
# def get_loss(x,x_e,margin=0.2):
#     loss_fn = nn.TripletMarginLoss(margin=margin)
#     #I T见
#     hard_neg_x_emb, hard_neg_x_e_emb = get_hard_negatives(x, x_e)
#     anchor_scores = torch.sum(x*x_e,dim=1)
#     neg_scores = torch.sum(hard_neg_x_emb*hard_neg_x_e_emb,dim=1)
#     loss_1=loss_fn(anchor_scores,neg_scores)
#     # T I见
#     hard_neg_x_emb_, hard_neg_x_e_emb_ = get_hard_negatives(x_e, x)
#     anchor_scores_ = torch.sum(x_e * x, dim=1)
#     neg_scores_ = torch.sum(hard_neg_x_emb_ * hard_neg_x_e_emb_, dim=1)
#     loss_2 = loss_fn(anchor_scores_, neg_scores_)
#
#     loss=loss_1+loss_2
#     return loss
#
# def get_hard_negatives(img_embs, sent_embs):
#     """
#     Computes hard negatives for the given batch of image-sentence pairs.
#
#     Args:
#         img_embs: tensor of shape (batch_size, img_emb_dim)
#         sent_embs: tensor of shape (batch_size, sent_emb_dim)
#         margin: margin for the triplet loss
#
#     Returns:
#         img_hard_negatives: tensor of shape (batch_size, img_emb_dim)
#         sent_hard_negatives: tensor of shape (batch_size, sent_emb_dim)
#     """
#     batch_size = img_embs.size(0)
#     sim_scores = img_embs @ sent_embs.t()
#
#     # set similarity scores for diagonal elements to a very small value
#     sim_scores.fill_diagonal_(-1e9)
#
#     # find the hard negatives for images
#     I = sim_scores.argmax(dim=1)  # get the index of the most similar sentence for each image
#     I_scores = sim_scores.gather(1, I.view(-1, 1)).squeeze()  # get the similarity scores for each I
#     I_neg_scores = sim_scores - I_scores.view(-1, 1)  # calculate the negative similarity scores for each image
#     I_neg_scores[I == torch.arange(batch_size)] = -1e9  # set the scores of matched pairs to a very small value
#     _, img_hard_neg_idx = I_neg_scores.max(dim=1)  # get the index of the hardest negative for each image
#     img_hard_negatives = sent_embs[img_hard_neg_idx]
#
#     # find the hard negatives for sentences
#     T = sim_scores.argmax(dim=0)  # get the index of the most similar image for each sentence
#     T_scores = sim_scores.gather(0, T.view(1, -1)).squeeze()  # get the similarity scores for each T
#     T_neg_scores = sim_scores - T_scores.view(1, -1)  # calculate the negative similarity scores for each sentence
#     T_neg_scores[T == torch.arange(batch_size)] = -1e9  # set the scores of matched pairs to a very small value
#     _, sent_hard_neg_idx = T_neg_scores.max(dim=0)  # get the index of the hardest negative for each sentence
#     sent_hard_negatives = img_embs[sent_hard_neg_idx]
#
#     return img_hard_negatives, sent_hard_negatives
def generateMat(k=10):
    ...
    """
    # 每次加载都把之前的数据删除
    filenames = os.listdir('data_WS/processed')
    print(filenames)
    for s in filenames:
        os.remove('data_WS/processed/' + s)
...
def split_data(tenfold=False):
    """
    读取./data_WS/processed/mask_mat.mat，根据原始频率矩阵生成10份被mask的频率矩阵并yield
    :return:
    """
    raw_frequency = scipy.io.loadmat(raw_file)
    raw = raw_frequency['R']
...
if __name__ == '__main__':
    ...
    parser.add_argument('--lamb', type=float, required=False, default=0.05, help='LAMBDA')
    ...
    modeling = [GCN, GIN, GAT, GAT3, GAT, RGCN][args.model]
    ...
    # processed_frequencyMat = 'data_WS/processed/frequencyMat.csv'
